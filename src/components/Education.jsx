  ...
  <div className="bg-white dark:bg-neutral-900/60 rounded-2xl border border-neutral-300 dark:border-white/20 p-6 transition-colors duration-500">
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-0">
      <div className="max-w-full md:max-w-2xl">
        <h3 className="text-xl sm:text-2xl font-semibold flex items-center gap-3 text-black dark:text-white">
        ...
        <p className="text-gray-800 dark:text-gray-200 text-base sm:text-lg font-medium mt-1 ml-0 md:ml-1">
          {org}
        </p>
        {desc && <p className="text-gray-600 dark:text-gray-400 text-sm sm:text-base mt-2">{desc}</p>}
      </div>
      <div className="flex flex-col items-start md:items-end text-sm sm:text-base text-gray-600 dark:text-gray-400 mt-2 md:mt-0 gap-1 leading-normal">
        <p>📅 {meta}</p>
...
export default function Education() {
  return (
    <section
      id="education"
      className="scroll-mt-16 pt-20 sm:pt-32 px-4 sm:px-6 pb-20 bg-[#FFF5F2] dark:bg-black transition-colors duration-500"
    >
